...
func (f *NewEnvironmentForm) View() string {
	...
	help := lipgloss.NewStyle().Foreground(lipgloss.Color("240")).Render("Tab switch  Enter next/newline  Ctrl+S save  Esc cancel")
	...
	b := strings.Builder{}
	b.WriteString(styleTitle.Render("New environment"))
	b.WriteString("\n")
	...
	if f.focused == 4 { btnSave = styleSel.Render("Save") }
	btnCancel := styleDef.Render("Cancel")
	if f.focused == 5 { btnCancel = styleSel.Render("Cancel") }
