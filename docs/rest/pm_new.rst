...
Use the project-level flags to seed defaults without editing configuration files:
...
* ``--environment-env-var`` may be repeated to add or override individual environment variables.
...
Use ``--yes`` to bypass confirmation prompts during project or environment creation. Combine it with ``--dry-run`` when you need to preview changes without writing files.
