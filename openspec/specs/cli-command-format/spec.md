# cli-command-format Specification

## Purpose
TBD - created by archiving change standardize-pm-command-format. Update Purpose after archive.
## Requirements
### Requirement: CLI Command Module Layout
All packages under `internal/adapters/handlers/cli/<command>` SHALL follow the same structure demonstrated by `pm edit`:
